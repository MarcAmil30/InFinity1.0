...
from MolToFile import MolToMol2Block, MolToMol2File
_HasSubstructMatchStr=rdchem._HasSubstructMatchStr
...
def inference_from_files(args):
    ...
    use_rdkit_coords = args.use_rdkit_coords if args.use_rdkit_coords != None else args.dataset_params[
        'use_rdkit_coords']
    #Changes to directories below:
    for i in os.listdir(os.path.join(args.inference_path, "ligand")):
        if 'ligand' in i:
            lig_name = i
            counter += 1
    if counter != 1: raise ValueError(f'More than than one ligand in directory')
        # lig_name = os.listdir(os.path.join(args.inference_path, "ligand"))
    if not os.path.exists(os.path.join(args.inference_path, "ligand")):
        raise ValueError(f'Path does not exist: {os.path.join(args.inference_path, "ligand")}')
    print(f'Trying to load {os.path.join(args.inference_path, "ligand")}')            
    lig = read_molecule(os.path.join(args.inference_path, "ligand", lig_name), sanitize=True)
    if lig != None:  # read mol2 file if sdf file cannot be sanitized
        used_lig = os.path.join(args.inference_path, "ligand", lig_name)
    if lig_name == []: raise ValueError(f'No ligand file found. The ligand file has to contain \'ligand\'.')
    if lig == None: raise ValueError(f'Ligand file could not be read: {lig_name}')
    names = os.listdir(os.path.join(args.inference_path,"proteins"))
    for idx, name in enumerate(names):
        print(f'\nProcessing {name}: complex {idx + 1} of {len(names)}')
        rec_name = [i for i in names if 'rec.pdb' in i or 'protein' in i][0]
        rec_path = os.path.join(args.inference_path, "proteins", rec_name)
        ...
        with torch.no_grad():
            ...
            if args.run_corrections:
                ...
                if name.endswith('.pdb'):
                    output_file = name[:-len('.pdb')]
                if args.output_directory:
                    conf = optimized_mol.GetConformer()
                    for i in range(optimized_mol.GetNumAtoms()):
                        x, y, z = coords_pred_optimized[i]
                        conf.SetAtomPosition(i, Point3D(float(x), float(y), float(z)))
                    block_optimized = MolToMol2Block(optimized_mol)
                    print(f'Writing prediction to {args.output_directory}/{output_file}.mol2')
                    with open(f'{args.output_directory}/{output_file}.mol2', "w") as newfile:
